...
import { highlight_prism } from "./prism_highlighter";
...
export function highlight(code: string, lang: string | undefined): Token[][] {
    ...
    const prism = highlight_prism(code, lang);
    if (!prism)
        return no_highlight(code);
    
    return mergeTokens(prism);
}
...
export function tokenTypeToCssClass(type: TokenType): string {
    switch (type) {
        ...
        case TokenType.Macro:         return "code_token_macro";
